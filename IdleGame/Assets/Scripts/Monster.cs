...
////컴포넌트(Component)
////유니티 오브젝트가 사용할 기능
////제공되는 컴포넌트가 있고, 스크립트의 경우는 사용자가 만들어주는 사용자 정의 컴포넌트로써 활용이 가능합니다.(Mono 상속)
...
//    bool isSpawn = false;
//    //몬스터 클래스에서 상황에 맞게 애니메이션을 실행 시키려 합니다.
//    //이때 필요한 데이터는 무엇일까요?
//    // Animator
...
//        float percent = 0.0f; //반복문의 종료조건
//        float start = 0.0f; //변화 시작값
//        float end = transform.localScale.x; // 변화 목표값
//        //localScale은 게임 오브젝트의 상대적인 크기를 의미
//        //현재는 오브젝트의 크기로 기억합니다.
//        while (percent < 1.0f)
//        {
...
//        //코드 내에서 Animator 로 인식하고, Animator 의 필드나 메소드를 사용할 수 있습니다.
//         // Monster가 실행할 Start 작업
//        MonsterInit();
//        HP = 5.0f;
//        GetDamage(5.0);
//    }
...
//        if (distance <= 1f)
//        {
//            SetAnimator("isIDLE"); // 대기모드로 변경.
//        }
//        else
//        {
//            transform.position = Vector3.MoveTowards(transform.position, Vector3.zero, speed * Time.deltaTime);
//            SetAnimator("isMOVE"); // 아니면 움직여
//        }
...
//        //4. Time.deltaTime : 이전 프레임이 완료되기 까지 걸린 시간 (컴퓨터가 느릴수록 값이 커짐) //일반적으로 약1초
//        // 업데이트에서 작업을 하는데 있어서의 보정 값 역할
//        //5. trasnform.LookAt(Vector3 posotion);
//        // 특정 방향을 바라보게 설정해 주는 기능
...
public class Monster : Unit
{
    ...
    public void GetDamage(double dmg)
    {
        ...
        if (HP <= 0)
        {
            ...
            // 아이템 드랍 기능 추가
            // 현재 아이템 테이블이 따로 구현이 안되있기 때문에 고정 값
            // 변수를 만들거나, 아후에 아이템 관련 데이터 추가해서 그만큼 처리하게 하자
            for (int i = 0; i < 4; i++)
            {
